...
Setelah ketiga topik terbahas, minta pengguna klik tombol "Kumpulkan & Rangkum Memori" di atas. Jangan melompat topik. Respons maksimal 2-3 kalimat per giliran agar percakapan terasa seperti ngobrol nyata.`;
  ...
  const finalizeChat = async () => {
    ...
    try {
      const rawText = await callGemini(
        'Kamu adalah ekstraktor data jurnal. Kembalikan HANYA objek JSON valid tanpa markdown, tanpa kode block, tanpa penjelasan apapun.',
        [{
          role: 'user',
  ...
  const submitForm = async (e) => {
    ...
    try {
      const rawText = await callGemini(
        `Kamu adalah "${aiCompanion.name}" ${aiCompanion.avatar}, asisten refleksi hangat dalam Bahasa Indonesia. Kembalikan HANYA JSON valid tanpa markdown: {"summary":"...","advice":"..."}`,
        [{
          role: 'user',
