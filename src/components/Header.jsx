...
import { BrainCircuit, Bell, Edit3, LogOut } from 'lucide-react';
import { useApp } from '../context/AppContext';
import { GOAL_OPTIONS } from '../data/constants';
import CompanionModal from './CompanionModal';

export default function Header() {
  const { user, aiCompanion, logout, triggerToast } = useApp();
      ...
      <header className="bg-[#FFFDF9] border-b-4 border-[#2C3E35] sticky top-0 z-40 shadow-[0_4px_0_0_rgba(44,62,53,1)]">
        <div className="max-w-6xl mx-auto px-4 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate(user ? '/dashboard' : '/')}>
            <div className="bg-[#7CA190] comic-border-thick p-2 rounded-2xl text-white comic-shadow-sm hover:rotate-6 transition-all duration-300">
              <BrainCircuit className="w-6 h-6 text-[#FFFDF9]" />
            </div>
            <div>
              <div className="flex items-center gap-1">
                <span className="font-comic-title font-black text-2xl tracking-tight text-[#2C3E35]">
                  Day<span className="text-[#EBB39C]">Mirror</span>
                </span>
                <span className="text-xl animate-bounce">🪞</span>
              </div>
              <p className="text-[10px] text-[#7CA190] font-black tracking-wider uppercase">Your Comic Diary & AI Companion</p>
            </div>
          </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                <button
                  ...
                  className="p-2.5 rounded-xl bg-white comic-border-thin hover:bg-[#F8F5EE] transition-all text-[#2C3E35] relative comic-shadow-sm hover:scale-105"
                >
                  <Bell className="w-5 h-5" />
                  {unread && <span className="absolute top-1 right-1 w-3.5 h-3.5 bg-[#EBB39C] rounded-full border-2 border-[#2C3E35]" />}
                </button>
                {showNotif && (
                  <div className="absolute right-0 mt-3 w-80 bg-[#FFFDF9] comic-border-thick rounded-3xl comic-shadow-md p-5 z-50">
              ...
              <div
                ...
                className="flex items-center gap-3.5 border-l-4 border-dashed border-[#2C3E35] pl-4 cursor-pointer group"
              >
                <div className="w-10 h-10 rounded-xl bg-[#F7D3C6] comic-border-thin text-[#7A3F35] font-black flex items-center justify-center comic-shadow-sm transform -rotate-3 text-base group-hover:scale-105 transition-transform">
                  {user.name.charAt(0).toUpperCase()}
                </div>
                <div className="hidden sm:block text-left">
                  <p className="text-xs font-black text-[#2C3E35] leading-3 group-hover:text-[#7CA190] transition-colors">{user.name}</p>
              ...
              <button
                onClick={logout}
                className="p-2 rounded-xl bg-[#FFF6E0] comic-border-thin hover:bg-[#FCE4D6] hover:text-[#843C0C] transition-all ml-1 comic-shadow-sm hover:scale-105"
                title="Logout"
              >
